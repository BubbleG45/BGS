...
**Kinetic Stadium visual restyle ported to `bgs_web` (2026-08-07):** the Jaspr public site now carries the same design tokens as the Flutter app -- own idioms though, not a literal port: Jaspr's type-safe CSS DSL instead of `google_fonts`/Material widgets, a plain `material-symbols-outlined` span (new `MaterialSymbol` component) instead of an icon package. Every page restyled (header, home, org homepage, league, event, search), same feature-audit exclusions carried over from the Flutter pass (no ratings, no team/player search, no capacity, no self-serve registration CTAs, no extra filters). Found and fixed one real bug in the process: a hardcoded `sports_volleyball` icon on every sport line, copied uncritically from the mockup's volleyball-only demo data -- swapped for a generic glyph since the real `Sport` enum has ~30 values with no icon mapping. See CHANGELOG.md for the full writeup.

**Next up:** Google/Apple Sign-In (a stated v1 goal in §1) still isn't wired server-side -- only email is registered as a provider in `server.dart`; needs OAuth credentials and platform configuration the user will need to provide before that can happen. Railway deployment is ready to attempt whenever the user has a Railway account/CLI session available -- but note it needs Serverpod Cloud (or some other public backend host) stood up first, since `bgs_web` deployed as-is would have nothing reachable to point `BGS_API_URL` at.

**Phase A -- reusable component system + real page structure (2026-08-07):** superseded the Kinetic Stadium visual restyle with a deeper rebuild: a shared `EntityCard`/grid component system on both Flutter and Jaspr (replacing eight-plus screens' worth of hand-rolled `Card`+`ListTile`/duplicated CSS), plus real product depth -- org pages split into Active/Upcoming/Past leagues and events, richer league/event detail pages (registration-info card, real registrant lists), role-aware dashboards (Manager gained real "Record score"/"Take attendance" actions), a real Profile screen (bio, real wins/losses stats, an org owner's path to a `OrgSettingsScreen` with a payments placeholder). A handful of small structural backend deltas shipped alongside it (league season/registration dates, `rulesUrl`, `LeagueEndpoint.complete`, `MatchAttendance`, `PlayerProfile`) -- see CHANGELOG.md for the full list and the widened team-manager permission model this uncovered was needed. This is explicitly Phase A of a longer roadmap (recorded in full in this session's plan and in the CHANGELOG entry) -- playoff brackets/league winners, registration-window enforcement, self-serve captain registration, and geo search are Phase B; player positions/stats are Phase C; live scorekeeping is Phase D; real Stripe onboarding is Phase E.

**Next up:** Phase B (playoff brackets + league winners) is the natural next self-contained follow-up. Google/Apple Sign-In and Railway deployment remain blocked on the user providing external accounts/credentials.
