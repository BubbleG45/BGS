...
**Next up:** the three dashboards (Player/Organizer/Manager) and basic search (by sport, by org/league/event name) are what's left in Phase 1 -- both composed-query work across the endpoints that already exist, not new domain models. Worth deciding up front whether dashboards live as new endpoint methods (e.g. `listMine`-style aggregation on the server, gathering a player's teams/events/registrations in one call) or are assembled client-side from calls the Flutter app already has -- leaning server-side to keep the Flutter app thin and avoid N+1-style chains of calls from the client.
