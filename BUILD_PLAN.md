...
**Next up:** Google/Apple Sign-In (a stated v1 goal in §1) still isn't wired server-side -- only email is registered as a provider in `server.dart`; needs OAuth credentials and platform configuration the user will need to provide before that can happen. A first Jaspr public page is still open, and is now the natural next step: with every Phase 1 feature area built in Flutter, Jaspr's job is to make the org homepage/league page/event page/search actually reachable by an anonymous visitor (not just a logged-in Flutter user), which is exactly the §7 "public means logged in" decision forcing itself to be made.
