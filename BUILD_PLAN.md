...
**Next up:** Google/Apple Sign-In (a stated v1 goal in §1) still isn't wired server-side -- only email is registered as a provider in `server.dart`; needs OAuth credentials and platform configuration the user will need to provide before that can happen. A first Jaspr public page is still open too, and still the way to de-risk Jaspr's pre-1.0 status (§7). With write-side UI now complete, the remaining Phase 1 checklist items (public org homepage, webcal-adjacent schedule display, basic search UI, event registration UI) are all read-side/public-facing -- worth doing together with the "public means logged in" open decision in §7, since they're the same audience.
