...
**Resume point (as of 2026-08-03):** domain model/schema built and migrated; `OrganizationEndpoint`, `LeagueEndpoint`, and `TeamEndpoint` are all live. Org/League/Team writes share `requireOrgRole` (`bgs_server/lib/src/organizations/organization_access.dart`); `TeamEndpoint.acceptInvite`/`declineInvite` introduced a second, simpler authorization pattern -- "is the calling AuthUser the one on this row" -- for the first player-side write in the app. Player lookup for invites goes through the auth module's `UserProfile.email` (queryable because `serverpod_auth_core_server`'s top-level export re-exports its full generated protocol, not just the DTOs). See CHANGELOG.md for specifics. Test pattern to copy: `AuthUsers().create()` + `AuthenticationOverride.authenticationInfo()` for authenticated test sessions, `UserProfiles().createUserProfile()` when a test user needs an email (for invite flows), `TestSessionBuilder` (not `Session`) as the type passed into generated `endpoints.x.method(...)` test wrappers.

**Next up:** `EventEndpoint` -- one-off events, including the "orgless" case (`organizationId` nullable, already modeled) and a shareable public event link. Likely reuses the public-read / admin-gated-write shape again, but creation needs to branch: org-scoped events still check `requireOrgRole`, while orgless events just need `requireLogin` and the creator becomes `createdByAuthUserId`. `EventRegistration` (a player registering for an event directly, no team involved) is a natural companion to build alongside it.
