...
**Next up:** Google/Apple Sign-In (a stated v1 goal in §1) still isn't wired server-side -- only email is registered as a provider in `server.dart`; needs OAuth credentials and platform configuration the user will need to provide before that can happen. A first Jaspr public page is still open too, and still the way to de-risk Jaspr's pre-1.0 status (§7). Beyond that, the dashboards are read-only overviews -- there's no in-app UI yet for the write side (creating orgs/leagues/teams/events, inviting players, scheduling matches); that's all still done via direct endpoint calls (or would be, for a real organizer) rather than through screens.
