...
**Next up:** Google/Apple Sign-In (a stated v1 goal in §1) still isn't wired server-side -- only email is registered as a provider in `server.dart`; needs OAuth credentials and platform configuration the user will need to provide before that can happen. On the Jaspr side: league/event public detail pages and a public search page are the natural next slice, following the same `PublicEndpoint` pattern now that it exists -- and Railway deployment is ready to attempt whenever the user has a Railway account/CLI session available.
