...
**Resume point (as of 2026-08-03):** domain model/schema built and migrated; `OrganizationEndpoint` and `LeagueEndpoint` are both live, sharing the same `requireOrgRole` authorization helper (`bgs_server/lib/src/organizations/organization_access.dart`) and the same shape: typed exceptions for not-found/slug-taken/invalid-transition, public reads, admin-gated writes, pre-checked uniqueness with the DB unique index as backstop. See CHANGELOG.md for specifics. Test pattern to copy: `AuthUsers().create()` + `AuthenticationOverride.authenticationInfo()` for authenticated test sessions, `TestSessionBuilder` (not `Session`) as the type passed into generated `endpoints.x.method(...)` test wrappers.

**Next up:** `TeamEndpoint` + the player-invite flow -- create a team within a league (`requireOrgRole(..., minRole: admin)`), invite a player (creates a `TeamMembership` with `status: invited`), and an accept/decline action callable by the invited player themselves (not an org-role check -- authorization here is "is this the AuthUser on the membership row", a different, simpler pattern worth introducing cleanly since it's the first player-side write in the app).
