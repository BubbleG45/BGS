...
**Next up:** the remaining Phase 1 items are all read/composition work rather than new domain models -- Profiles (thin wrapper over the auth module's existing `UserProfile`), the three dashboards (Player/Organizer/Manager -- likely composed queries across the endpoints that already exist, not new endpoints or models), and basic search (by sport, by org/league/event name). Worth deciding up front whether dashboards live as new endpoint methods (e.g. `listMine`-style aggregation on the server) or are assembled client-side from calls the Flutter app already has -- leaning server-side to keep the Flutter app thin and avoid N+1-style chains of calls from the client.
