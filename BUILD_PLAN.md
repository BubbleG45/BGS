...
**Resume point (as of 2026-08-03):** domain model/schema built and migrated; `OrganizationEndpoint`, `LeagueEndpoint`, `TeamEndpoint`, and `EventEndpoint` are all live. Org/League/Team/Event writes share `requireOrgRole` (`bgs_server/lib/src/organizations/organization_access.dart`) for org-scoped resources; two narrower "is this your own row" patterns now exist alongside it -- `TeamMembership.acceptInvite`/`declineInvite` (player accepting/declining their own invite) and `Event.update`/`publish` for *orgless* events (creator-only, since there's no org role to check). Player lookup for team invites goes through the auth module's `UserProfile.email` (queryable because `serverpod_auth_core_server`'s top-level export re-exports its full generated protocol, not just the DTOs). See CHANGELOG.md for specifics. Test pattern to copy: `AuthUsers().create()` + `AuthenticationOverride.authenticationInfo()` for authenticated test sessions, `UserProfiles().createUserProfile()` when a test user needs an email (for invite flows), `TestSessionBuilder` (not `Session`) as the type passed into generated `endpoints.x.method(...)` test wrappers.

**Model-yaml gotcha found and fixed:** a nullable relation field (e.g. `organization: Organization?, relation(onDelete=Cascade)`) only makes the *loaded object reference* nullable -- the underlying foreign-key id column still generates as required unless you also add `relation(optional)`. `Event.organizationId` had this bug from the original domain-model pass; caught it before building `EventEndpoint` on top of it. Worth double-checking `ScheduledMatch`'s `homeScore`/`awayScore`-adjacent relations and anything else meant to be optional when picking those back up.

**Next up:** manual schedule creation (`ScheduledMatch` -- no auto-scheduling algorithm yet, just an org admin picking two teams, a time, and a location within a league) and basic standings (`Standing` -- win/loss/tie record, likely recomputed on match completion rather than stored incrementally at first). After that, Phase 1's remaining items are Profiles, the three dashboards (likely composed read-queries across the endpoints that already exist, not new endpoints), and basic search.
