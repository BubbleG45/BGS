...
**Next up:** basic search (by sport, by org/league/event name) is the last item on the Phase 1 list (§5). After that, Phase 1's broad-but-shallow slice is functionally complete on the backend -- worth pausing to take stock of what's *not* yet done before diving into Phase 2: no Flutter UI has been built against any of this yet (everything so far is server + tests), and the public Jaspr site is still just the Phase 0 spike. Both are probably higher-value next moves than polishing search further.
